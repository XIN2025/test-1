...
const PermissionsPage = () => {
  ...
  const authenticateAndContinue = async () => {
    ...
    if (email && name) {
      ...
      Alert.alert(
        "Missing Information",
        "User email or name not found. Cannot continue.",
      );
    }
  };
  ...
  const registerForPushNotificationsAsync = async () => {
    ...
    try {
      ...
      if (!email) {
        Alert.alert(
          "Error",
          "User email not found. Cannot register FCM token.",
        );
        return;
      }

      try {
        const { success, message } = await nudgeApi.registerFcmToken(
          String(email),
          token,
        );
        if (!success) {
        ...
        Alert.alert(
          "Error",
          err.message || "Failed to register FCM token with backend.",
        );
        return;
      }
      ...
      Alert.alert(
        ...
        "You will now receive nudges and reminders.",
      );
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F9FAFB" }}>
      ...
      >
        ...
        >
          ...
          >
            To help you stay on track, Evra would like to send you notifications
            for nudges and reminders.
          </Text>
          ...
          {permissionStatus === "denied" && (
            ...
            >
              ...
              >
                Notifications are disabled. You can enable them in your device
                settings.
              </Text>
            </View>
          )}
