...
interface AndroidHealthCardProps {
  isDarkMode?: boolean;
  onPress?: () => void;
  width?: number;
  height?: number;
...
export default function AndroidHealthCard({
  isDarkMode = false,
  onPress,
  width = 160,
  height = 160,
}: AndroidHealthCardProps) {
  ...
  const checkPermissionsStatus = useCallback(async () => {
    if (hasPermissions && isAvailable) {
      try {
        ...
        console.error('Error verifying permissions in AndroidHealthCard:', error);
      }
    }
